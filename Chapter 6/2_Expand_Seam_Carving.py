...
#Add vertical seam to the image
def add_vertical_seam(img, seam, num_iter):
    ...
    for row in range(rows):
        ...
        #value of the neighbors. It looks smooth this way and 2
        #can avoid unwanted artifacts

        for i in range(3):
            v1 = img_extended[row, int(seam[row])-1, i]
            v2 = img_extended[row, int(seam[row])+1, i]
            img_extended[row, int(seam[row]), i] = (int(v1)+int(v1))/2
    
    return img_extended


if __name__ == "__main__":
    img_input = cv2.imread('D:/A LEARN/Image Processing/A Panduan Kang Gip/image/duck.jpg')
    num_seams = int(50)

    img = np.copy(img_input)
    img_output = np.copy(img_input)
    img_overlay_seam = np.copy(img_input)
    energy = compute_energy_matrix(img) #Same than previous code


    for i in range(num_seams):
        seam = find_vertical_seam(img, energy) #Same than previous code
